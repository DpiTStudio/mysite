...
ALLOWED_HOSTS = [
    ...
    '127.0.0.1:4234', # Локальный хост с портом
    'localhost', # Локальный хост
    'localhost:4234', # Локальный хост с портом
    '46.149.71.34', # IP адрес
    '46.149.71.34:4234', # IP адрес с портом
    '192.168.0.4', # IP адрес локальной сети
    '192.168.0.4:4234', # IP адрес локальной сети с портом
    'dpit-cms.ru', # Домен
    'dpit-cms.ru:4234', # Домен с портом
    'www.dpit-cms.ru', # Домен с www
] + env.list("ALLOWED_HOSTS", default=["*"])  # Дополнительные хосты из .env (через запятую)
