...
class ServiceAdmin(admin.ModelAdmin):
    ...
    def icon_preview(self, obj):
        if obj.icon:
            return format_html(
                '<img src="{}" style="height: 28px; width: 28px; object-fit: contain; border-radius: 6px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 2px;" />',
                obj.icon.url,
            )
        return format_html(
    ...
    def category_price_display(self, obj):
        ...
        if obj.price_type == "fixed" and obj.price_fixed:
            ...
            price_html = format_html(
                '<span style="color: #fbbf24; font-weight: 600;">Договорная</span>'
            )
        else:
            price_html = format_html(
                '<span style="color: #cbd5e1; font-weight: 600;">Уточняйте</span>'
            )
